...
export function DaySection({ day, nextDay }: { day: TripDay; nextDay?: TripDay }) {
  ...
  return <section className="day" id={day.id}>
    ...
    <div className="day-places"><span className="section-label">Lugares del día</span><ul>{day.placesVisited.map((place) => <li key={`${day.id}-${place.name}`}>
      {place.slug ? <Link href={`/viajes/india/lugares/${place.slug}`} className="place-link"><span className="place-link-copy"><strong>{place.name}</strong><span>{place.region}</span></span><span aria-hidden="true">↗</span></Link> : <div className="place-link place-link--static"><span className="place-link-copy"><strong>{place.name}</strong><span>{place.region}</span></span></div>}
    </li>)}</ul></div>
